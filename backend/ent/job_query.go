...
func (jq *JobQuery) QueryPatients() *PatientQuery {
...
func (jq *JobQuery) WithPatients(opts ...func(*PatientQuery)) *JobQuery {
	query := &PatientQuery{config: jq.config}
	for _, opt := range opts {
		opt(query)
	}
	jq.withPatients = query
	return jq
}
...
func (jq *JobQuery) sqlAll(ctx context.Context) ([]*Job, error) {
	...
	if query := jq.withPatients; query != nil {
		...
		query.Where(predicate.Patient(func(s *sql.Selector) {
			s.Where(sql.InValues(job.PatientsColumn, fks...))
		}))
