...
func (tq *TitleQuery) QueryPatients() *PatientQuery {
...
func (tq *TitleQuery) WithPatients(opts ...func(*PatientQuery)) *TitleQuery {
	query := &PatientQuery{config: tq.config}
	for _, opt := range opts {
		opt(query)
	}
	tq.withPatients = query
	return tq
}
...
func (tq *TitleQuery) sqlAll(ctx context.Context) ([]*Title, error) {
	...
	if query := tq.withPatients; query != nil {
		...
		query.Where(predicate.Patient(func(s *sql.Selector) {
			s.Where(sql.InValues(title.PatientsColumn, fks...))
		}))
